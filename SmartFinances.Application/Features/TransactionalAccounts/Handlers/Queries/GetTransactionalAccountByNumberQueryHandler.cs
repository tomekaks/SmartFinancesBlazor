...
{
    public class GetTransactionalAccountByNumberQueryHandler 
        : IRequestHandler<GetTransactionalAccountByNumberQuery, TransactionalAccountDto>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ITransactionalAccountFactory _transactionalAccountFactory;

        public GetTransactionalAccountByNumberQueryHandler(IUnitOfWork unitOfWork, ITransactionalAccountFactory transactionalAccountFactory)
        {
            _unitOfWork = unitOfWork;
            _transactionalAccountFactory = transactionalAccountFactory;
        }

        public async Task<TransactionalAccountDto> Handle(GetTransactionalAccountByNumberQuery request, CancellationToken cancellationToken)
        {
