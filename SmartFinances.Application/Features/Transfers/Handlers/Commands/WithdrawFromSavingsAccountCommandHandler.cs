...
{
    public class WithdrawFromSavingsAccountCommandHandler : IRequestHandler<WithdrawFromSavingsAccountCommand>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ITransferFactory _transferFactory;

        public WithdrawFromSavingsAccountCommandHandler(IUnitOfWork unitOfWork, ITransferFactory transferFactory)
        {
            _unitOfWork = unitOfWork;
            _transferFactory = transferFactory;
        }

        public async Task Handle(WithdrawFromSavingsAccountCommand request, CancellationToken cancellationToken)
        {
            ...
            var transfer = _transferFactory.MapFromWithdrawal(request.TransferDto);
            ...
            if (savingsAccount.Balance >= request.TransferDto.Amount)
            {
